...
export default function Hero({ onNavTransform, isNavbarMode }) {
  ...
  const socialLinks = [
    { name: "GitHub", url: "https://github.com/rudresh051", icon: "github" },
  ...
  return (
    <>
      {/* Dual Navbar System when in navbar mode */}
      <AnimatePresence>
        {isNavbarMode && (
          <div className="fixed top-6 left-[5vw] right-[5vw] z-[9999] flex items-stretch gap-3">
      ...
      <section id="home" className="relative min-h-screen flex items-center justify-center px-8 md:px-16 lg:px-24 xl:px-32 pt-20">
        <div className="w-full max-w-7xl grid grid-cols-1 lg:grid-cols-5 gap-8 lg:gap-12 items-start lg:items-center">
          {/* Left Side - Profile & Menu (or placeholder when navbar active) */}
          ...
          <AnimatePresence>
            {!isNavbarMode && (
              <motion.div
                ...
                exit={{ opacity: 0, x: 50 }}
                transition={{ duration: 0.5 }}
                className="lg:col-span-3 flex flex-col space-y-8 text-center lg:text-left"
              >
                {/* Rotating Greeting */}
                <motion.div
